...
const App = () => {
  ...
  const setSize = (cuisine_type) => {
    const sizes = [...selectedSizes];
    
    if(sizes.includes(cuisine_type)) {
      const index = sizes.indexOf(cuisine_type);
      sizes.splice(index, 1);
    }
    else {
      sizes.push(cuisine_type);
    }
    setSelectedSizes(sizes);
    setProducts(filterList(sizes, 'cuisine_type'));
  }
